...
function IncidentListItem({
  ...
  return (
    <div
      ...
      onMouseEnter={e => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = '#1A3A6A'; }}
      onMouseLeave={e => { if (!selected) (e.currentTarget as HTMLElement).style.borderColor = '#0D2244'; }}
    >
      ...
      <div style={{ fontSize: 12, color: '#C8D8E8', marginBottom: 6, lineHeight: 1.4 }}>{inc.title}</div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 10, color: '#4A6A8A', fontFamily: 'JetBrains Mono, monospace' }}>
          ⏱ {inc.elapsed}
        </span>
        <span style={{ fontSize: 10, color: sc, fontFamily: 'JetBrains Mono, monospace' }}>
          {inc.node}
        </span>
      </div>
    </div>
    ...
    <div style={{ height: '100%', overflowY: 'auto', padding: 20 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#00D4FF', fontFamily: 'JetBrains Mono, monospace' }}>
            {inc.id}
          </span>
          <GlowBadge severity={inc.severity === 'info' ? 'info' : inc.severity} label={inc.severity.toUpperCase()} />
          <GlowBadge severity={inc.type === 'attack' ? 'critical' : 'warning'} label={inc.type.toUpperCase()} />
        </div>
        <div style={{ fontSize: 16, fontWeight: 600, color: '#C8D8E8', marginBottom: 4 }}>{inc.title}</div>
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          {[
            ...
            <div key={kv.label} style={{ display: 'flex', gap: 5 }}>
              <span style={{ fontSize: 11, color: '#4A6A8A' }}>{kv.label}:</span>
              <span style={{ fontSize: 11, color: (kv as { color?: string }).color ?? '#C8D8E8', fontFamily: 'JetBrains Mono, monospace' }}>
                {kv.value}
              </span>
            </div>
          ))}
      ...
      {/* Root cause */}
      <div style={{ background: '#06111F', border: '1px solid #0D2244', borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: '#4A6A8A', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
          🧠 Root Cause (Causal GNN)
        </div>
        <div style={{ fontSize: 12, color: '#C8D8E8', lineHeight: 1.6 }}>{inc.rootCause}</div>
        <div style={{ marginTop: 10 }}>
          <MetricBar label="GNN Confidence" value={Math.round(inc.confidence)} color="#9B5DE5" height={6} />
        </div>
      </div>

      {/* Affected services */}
      <div style={{ background: '#06111F', border: '1px solid #0D2244', borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: '#4A6A8A', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>
      ...
      {/* Autonomous action */}
      <div style={{ background: '#06111F', border: '1px solid #00FF9F33', borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: '#4A6A8A', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>
          ⚡ Autonomous Action
        </div>
        <div style={{ fontSize: 12, color: '#00FF9F', lineHeight: 1.6 }}>{inc.autoAction}</div>
      </div>

      {/* Timeline */}
      <div style={{ background: '#06111F', border: '1px solid #0D2244', borderRadius: 10, padding: 16 }}>
        ...
        {inc.timeline.map((evt, i) => (
          <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ fontSize: 16 }}>{evt.icon}</div>
              {i < inc.timeline.length - 1 && (
                <div style={{ width: 1, height: 14, background: '#0D2244', marginTop: 3 }} />
              )}
            </div>
            <div style={{ paddingTop: 1 }}>
              <div style={{ fontSize: 10, color: '#4A6A8A', fontFamily: 'JetBrains Mono, monospace', marginBottom: 2 }}>
                {evt.time}
              </div>
              <div style={{ fontSize: 12, color: '#C8D8E8', lineHeight: 1.4 }}>{evt.event}</div>
            </div>
          </div>
        ))}
      </div>
